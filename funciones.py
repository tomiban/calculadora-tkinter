...
def borrar_todo_historial(lista, historial):
    # Muestra un mensaje de confirmacion. Si se presiona NO imprime operacion cancelada sino borra y muestra un mensaje de confirmacion
    msg = msg_askyesno()
    if not msg:
...
def borrar_historial(lista, historial):
    # Obtenemos los indices que ubican a la seleccion en la listbox
...
def reciclar_historial(lista,  visor):
    ...
    if indices:
        ...
        # Cortamos la operacion en el primer espacio (2+2 | = 4)
...
def calcular_expresion(expresion):
    ...
    try:
        # Verifica si la expresión contiene caracteres no válidos.
        for char in expresion:
            if char not in nros_validos and char not in operadores_validos:
                return "Invalid Expression"  # Devuelve un mensaje de error.

        # Si la expresión contiene "^2", calcula la potencia cuadrada.
        if "^2" in expresion:
            base = expresion.replace("^2", "")  # Elimina "^2" de la expresión.
        ...
        # Si no se encuentran casos especiales, evalúa la expresión.
        else:
